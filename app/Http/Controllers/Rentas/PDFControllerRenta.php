...
class PDFVentas extends FPDF
{
    ...
    function DatosPago($fecha, $anticipo, $total, $cantidad, $idCliente)
    {
        ...
        $this->Cell(0, 6, utf8_decode("Concepto: Renta material"), 0, 1, 'L', true);
    ...
    function DatoPersona($nombre, $calle, $colonia, $celular)
    {
        // Arial 12
        $this->SetFont('Arial', 'B', 12);
        // Color del borde
    ...
    function tituloTabla()
    {
        ...
        $this->setFont('Arial', 'B', 12);
        // Titulo
        $this->Cell(0, 6, "Datos de la renta", 0, 1, 'L');
    }
    ...
    function ImprimirDatosCompra($nombre, $calle, $colonia, $celular, $idCliente)
    {
        ...
        $mRentas = DB::table('renta')
            ->join('detalle_renta', 'detalle_renta.renta_id', '=', 'renta.id')
            ->join('producto', 'detalle_renta.producto_id', '=', 'producto.id')
            ->select(
                'producto.nombre',
                'renta.fechaRegistro',
                'renta.fechaInicio',
            ...
            ->where('renta.cliente_id', '=', $idCliente)->get();

        foreach ($mRentas as $item) {
...
class PDFControllerRenta extends Controller
{
    private $fpdf;
    ...
    public function createPDFRentasCompra($idCliente, $idPersona)
    {
        $mCliente = Clientes::find($idCliente);
